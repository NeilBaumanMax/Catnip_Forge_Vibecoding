...
export function providerModelEndpoints(provider: ProviderConfig): URL[] {
  if (!provider.claudeCode) throw new Error('当前供应商没有 Claude Code 配置');
  if (provider.claudeCode.modelsUrl) return [new URL(provider.claudeCode.modelsUrl)];
  const base = new URL(provider.baseUrl.replace(/\/+$/, ''));
  const roots = [base.toString().replace(/\/+$/, '')];
  if (/\/anthropic$/i.test(base.pathname)) {
    const withoutAnthropic = new URL(base);
    withoutAnthropic.pathname = withoutAnthropic.pathname.replace(/\/anthropic$/i, '') || '/';
    roots.push(withoutAnthropic.toString().replace(/\/+$/, ''));
  }
  return [...new Set(roots.flatMap((root) => [`${root}/v1/models`, `${root}/models`]))].map((value) => new URL(value));
}

async function fetchModelList(endpoint: URL, provider: ProviderConfig, credential: string, fetchImpl: typeof fetch): Promise<Response> {
...
export async function discoverProviderModels(
  ...
  const endpoints = providerModelEndpoints(provider);
  let response: Response | null = null;
  let lastConnectionError = false;
  for (const endpoint of endpoints) {
    try {
      const candidate = await fetchModelList(endpoint, provider, credential, fetchImpl);
      if (candidate.status === 401 || candidate.status === 403) throw new Error(`供应商拒绝模型列表请求（HTTP ${candidate.status}）`);
      if (candidate.ok) { response = candidate; break; }
    } catch (error) {
      if (error instanceof Error && /HTTP (?:401|403)/.test(error.message)) throw error;
      if (error instanceof Error && error.name === 'AbortError') throw new Error('获取可用模型超时');
      lastConnectionError = true;
    }
  }
  if (!response) throw new Error(lastConnectionError ? '无法连接供应商的模型列表接口' : '供应商没有可用的模型列表接口；请填写模型列表 URL 或手动填写模型');
