...
import { Boxes, Code2, Compass, FolderOpen, GraduationCap, Minus, MonitorUp, Settings, Square, Workflow, X } from 'lucide-react';
import WorkspacePanel from './WorkspacePanel';
import CodeEditor from './CodeEditor';
import ExplorePanel, { type ExploreDiagnosisSeed } from './ExplorePanel';
import TaskControlsPanel from './task-manager/TaskControlsPanel';
    ...
    <div className={`browser-panel browser-panel--${mode} nes-container is-rounded`}>
      ...
      {mode === 'tasks' ? (
        <div className="task-manager-panel" data-tour-id="panel-tasks">
          <TaskControlsPanel
            buildLabel={UI_BUILD_LABEL}
            runtimeState={runtimeState}
            activeProjectName={activeProject?.name || null}
            hasSelectedProject={Boolean(activeProject && projectDir)}
            devices={hardboardDevices}
            selectedPort={selectedDevicePort || serialPort}
            onRefreshProjects={onRefreshWorkbench}
            onSelectProject={requestProjectChange}
            onBuild={handleManualBuild}
            onRefreshDevices={onRefreshHardboardDevices}
            onSelectPort={(port) => { setSelectedDevicePort(port); setSerialPort(port); }}
            onFlash={handleManualFlash}
          />
