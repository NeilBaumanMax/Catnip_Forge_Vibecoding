...
import { ArrowRight, BookMarked, Box, Code2, Cpu, ExternalLink, FolderOpen, Hammer, Lightbulb, ListChecks, MessageCircle, PenLine, PlayCircle, RefreshCw, Search, SearchCheck, Sparkles, Terminal } from 'lucide-react';
...
import ExploreSessionHistory from './explore/ExploreSessionHistory';
import ExploreKnowledgePreview from './explore/ExploreKnowledgePreview';
import { ExploreDiagnosisResults, ExploreIdeaResults } from './explore/ExploreAnalysisResults';
import ExploreConnectionStatus from './explore/ExploreConnectionStatus';
import ExploreOutputEmptyState from './explore/ExploreOutputEmptyState';
import ExplorePlanView from './explore/ExplorePlanView';
...
export default function ExplorePanel({ projectId, currentProject, hardwareSummary, runtimeSummary, diagnosisSeed }: Props) {
  ...
  const connectionBadge = (
    <ExploreConnectionStatus
      connection={connection}
      checking={checkingConnection}
      installing={installingConnection}
      starting={startingConnection}
      onInstall={installConnection}
    ...
    return (
      <section className="explore-panel" data-tour-id="panel-explore" aria-labelledby="explore-title">
        ...
        <ExploreKnowledgePreview
          ...
          setExpandedKnowledgeId={setExpandedKnowledgeId}
          beginVerification={beginVerification}
          deleteKnowledgeCard={deleteKnowledgeCard}
          setVerificationSummary={setVerificationSummary}
          setVerificationEvidence={setVerificationEvidence}
          saveVerification={saveVerification}
          setVerificationCardId={setVerificationCardId}
        />
  ...
  const planView = planPending || planResult ? (
    <ExplorePlanView
      ...
      onViewHandoff={() => selectStage('execute')}
    />
  ) : null;
      ...
      <form className="explore-form" onSubmit={(event) => void prepareRequest(event)}>
        ...
        <section className="explore-output-pane" aria-live="polite" aria-label="探索输出">
          ...
          {displayStage === 'describe' && !analysisPending && !analysisResult ? (
            <ExploreOutputEmptyState mode={isIdea ? 'idea' : 'diagnosis'} />
          ) : null}
