...
class ConnectionHandler:
    ...
    def _initialize_components(self):
        try:
            ...
            if self.vad is None:
                self.vad = self._vad
            if self.asr is None and os.environ.get("FORGE_TEXT_ONLY") != "1":
            ...
            if os.environ.get("FORGE_RADIO") == "1":
                asyncio.run_coroutine_threadsafe(
                    self.websocket.send(json.dumps({"type": "forge_ready"})), self.loop
                )
    ...
    def _initialize_intent(self):
        ...
        if hasattr(self, "loop") and self.loop:
            init_future = asyncio.run_coroutine_threadsafe(self.func_handler._initialize(), self.loop)
            if os.environ.get("FORGE_RADIO") == "1":
    ...
    def chat(self, query, depth=0):
        if depth == 0 and os.environ.get("FORGE_RADIO") == "1":
            try:
                from plugins_func.functions.forge_zhihu import persona_prompt
                self.change_system_prompt(persona_prompt())
            except Exception:
                asyncio.run_coroutine_threadsafe(
                    self.websocket.send(json.dumps({"type": "forge_error", "message": "无法读取 Forge 人设，请检查 Python 服务"})), self.loop
                )
                return
