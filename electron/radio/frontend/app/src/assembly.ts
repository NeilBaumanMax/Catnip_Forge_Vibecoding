...
if(new URLSearchParams(location.search).has('embedded'))document.body.classList.add('forge-embedded');
...
const wheelHost=document.createElement('div');el('palette').before(wheelHost);
...
// The API deliberately reuses UI events/functions; mouse and Agent share one rig.
function agentState(){return {version:'1.0',simulationOnly:true,loaded,model:el<HTMLSelectElement>('shell').value,pose:{...pose},colors:{...colors},mounts:{...config},explode,action,motionTimeMs:motionTime,motionStatus:el('motion-status').textContent,rotation:rig.rotation.y,rotate:spinning,wireframe:el('wireframe').getAttribute('aria-pressed')==='true',camera:{position:camera.position.toArray(),target:controls.target.toArray()},assembly:{inspect:el<HTMLSelectElement>('inspect').value,shell:checked('show-shell'),body:checked('show-body'),screen:checked('show-screen'),grip:checked('show-grip'),axes:checked('show-axes'),transparent:checked('transparent')},screen:{mode:screenMode,face:el<HTMLSelectElement>('face-select').value,status:screenMode==='js'?jsStatus:el('face-status').textContent}};}
function inputValue(id:string,value:unknown,event='input'){const input=el<HTMLInputElement>(id);input.value=String(value);input.dispatchEvent(new Event(event,{bubbles:true}));}
async function executeAgent(raw:unknown){
 const command=validateCommand(raw);const p=command.params as Record<string,any>;if(!loaded)throw Error('Models are still loading; retry when state.loaded is true');
 switch(command.type){
  case 'pose.set':for(const [key,value] of Object.entries(p))inputValue('joint-'+key,value);break;
  case 'colors.set':for(const [key,value] of Object.entries(p))setColor(key as ColorPart,String(value));break;
  case 'model.set':inputValue('shell',p.model,'change');break;
  case 'mount.set':for(const [key,value] of Object.entries(p))inputValue(key==='explode'?'explode':'mount-'+key,value);break;
  case 'assembly.view':for(const [key,value] of Object.entries(p)){if(key==='inspect')inputValue('inspect',value,'change');else{const target=el<HTMLInputElement>(key==='transparent'?key:'show-'+key);target.checked=Boolean(value);target.dispatchEvent(new Event('change',{bubbles:true}));}}break;
  case 'view.set':if(p.view)el(p.view).click();for(const [key,id] of [['rotate','orbit'],['wireframe','wireframe']])if(key in p&&Boolean(p[key])!==(el(id).getAttribute('aria-pressed')==='true'))el(id).click();break;
  case 'motion.play':for(const [key,value] of Object.entries({period:1000,steps:3,height:30,direction:1,...p}))if(['period','steps','height','direction'].includes(key))inputValue('firmware-'+key,value);inputValue('motion-speed',p.speed??1);playMotion(p.name);break;
  case 'motion.pause':el('pause-motion').click();break;
  case 'motion.home':home();el('motion-status').textContent='已复位';break;
  case 'screen.gif':selectScreen('gif',false);el<HTMLSelectElement>('face-select').value=p.face;await setFace('./faces/'+p.face+'.gif',true);break;
  case 'screen.js':
   el<HTMLTextAreaElement>('js-code').value=p.code;selectScreen('js',false);jsScreen.run(p.code);
   for(let i=0;i<60&&jsStatus==='正在编译 JavaScript…';i++)await new Promise(r=>setTimeout(r,50));
   if(!jsStatus.startsWith('JavaScript 动画运行中'))throw Error(jsStatus);break;
  case 'screen.stop':el('js-stop').click();jsStatus='已停止 · 保留最后一帧';break;
  case 'snapshot':{
   renderer.render(scene,camera);const canvas=document.createElement('canvas'),scale=Math.min(1,1024/Math.max(renderer.domElement.width,renderer.domElement.height));canvas.width=Math.round(renderer.domElement.width*scale);canvas.height=Math.round(renderer.domElement.height*scale);canvas.getContext('2d')!.drawImage(renderer.domElement,0,0,canvas.width,canvas.height);
   return {state:agentState(),mimeType:'image/png',image:canvas.toDataURL('image/png'),scope:'3d-viewport-only'};
  }
 }
 return {state:agentState()};
}
installAgentBridge({state:agentState,execute:executeAgent});
