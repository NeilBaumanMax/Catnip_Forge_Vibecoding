...
const taskHistoryPanel = fs.readFileSync(path.join(root, 'src/renderer/components/task-manager/TaskHistoryPanel.tsx'), 'utf8');
...
assert.match(browser, /onAnalyzeFailure=\{analyzeFailedTask\}/);
...
assert.match(taskHistoryPanel, /data-tour-id="task-analyze-problem"/);
assert.match(taskHistoryPanel, /task\.status === 'failed'/);
