...
const skillCall = JSON.stringify({
  type: 'assistant',
  message: { content: [
    { type: 'tool_use', id: 'tool-2', name: 'Skill', input: { skill: 'espidf-hardboard' } },
  ] },
});

const chunks = buffer.feed(`${toolCall}\n${toolResult}\n${skillCall}\n`);
assert.deepEqual(chunks.map((chunk) => chunk.type), ['text', 'tool_call', 'tool_result', 'tool_call']);
...
assert.equal(chunks[3].toolName, 'Skill');
assert.equal(chunks[3].content, '• Skill @espidf-hardboard');
