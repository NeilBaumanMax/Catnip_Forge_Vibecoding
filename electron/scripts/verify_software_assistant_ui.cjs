...
async function main() {
  ...
  try {
    ...
    if (!result?.triggerImageLoaded || !result?.fullBodyTrigger || !result?.popoverVisible || !result?.textarea || result?.actionButtons !== 7 || !result?.onboardingButton || !result?.sizeAdjusted
      || result?.title !== '学院呱呱' || !result?.welcome?.includes('Catnip Forge') || !inViewport) {
