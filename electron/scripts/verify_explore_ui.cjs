...
assert.match(browserPanel, /useState<PanelMode>\(\(\) => startsInHarnessMode \? 'repo' : 'explore'\)/, 'configured desktop startup must render Explore without a Model flash');
assert.match(browserPanel, /if \(startInCustomModelSetup\) setMode\('models'\)/, 'the startup alternative must route directly to Model management');
