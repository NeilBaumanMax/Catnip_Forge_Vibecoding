...
## 复开：并列成品误验收与光标仍偏移（2026-08-07）

用户在实际界面再次截图证明光标仍比末尾文字向右偏移。通过当前 Renderer CDP 直接读取确认：运行实例来自旧的 `electron/dist-package-fixed/win-unpacked`，其中 textarea 为 14px / 20.3px，而高亮层仍为 13px / 18.85px；33 个字符累计形成约 31px 偏移。原交付目录虽已包含 14px 修复，但保留旧并列成品造成误启动，且 smoke 只按窗口标题连接、没有验证 `app.asar` 所属路径，因此此前结论不能覆盖用户当前实例。

本次复开必须按以下门禁施工：

1. packaged UI smoke 必须校验 CDP target URL 属于指定成品根目录；发现同标题的其他成品实例时明确失败，禁止误连后报告通过。
2. textarea 与高亮层除宽度、字号和行高外，还必须统一并断言完整 `font` shorthand、字重、字形、字宽、字距、词距、kerning、字体特性和 `text-rendering`。
