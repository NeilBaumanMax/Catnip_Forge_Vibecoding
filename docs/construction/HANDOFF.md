...
- 首次配置不锁定后续设置：两类用户都可在“模型”标签页查看当前供应商、主模型和凭据状态，替换/清除 DeepSeek、千问或自定义供应商 Key，修改模型映射并切换活动供应商。
- 18g 源码已实现并通过专项、类型、Runtime 与真实 Electron 布局检查；双 Key 与单 Key WPF 均实际到达 `ContentRendered`。真实 Key 保存、付费模型请求及完整 Windows 包仍未验证。
