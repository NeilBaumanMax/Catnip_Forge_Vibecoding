...
| A6 | 官方CLI在Windows开发和打包版可运行 | 最新 Windows 候选、packaged Skill、release/version、Phase 6e app.asar 标记与隔离冷启动通过；全新用户真实安装/连接尚未人工验收 | 全新用户安装或 Secret 弹窗仍可能受机器环境影响 | 在不影响当前凭据的全新 Windows 用户环境实测安装授权→Secret 弹窗→status | TESTING | 候选 4,464,576,308 字节且不含用户 CLI/Key；开发环境 status 与真实搜索通过；Phase 6e 连接专项和成品标记通过 | Official Skill/Packaging |
