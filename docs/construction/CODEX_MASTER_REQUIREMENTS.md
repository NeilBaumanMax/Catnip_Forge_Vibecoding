...
| A6 | 官方CLI在Windows开发和打包版可运行 | 已有 Windows 候选的 packaged Skill、冷启动和 release 门禁通过；Phase 6e 首次安装/Secret 向导源码通过离线门禁但尚未重打实测 | 全新用户可能无法完成安装与连接 | 重打最新源码并实测安装授权→Secret 弹窗→status | TESTING | 已验证候选含官方 Skill 15 文件且不含用户 CLI/Key；开发环境 status 与真实搜索通过；Phase 6e 连接专项通过 | Official Skill/Packaging |
