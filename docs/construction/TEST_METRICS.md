...
| ??/?? | ?? | ?? | ??? | ?? |
| --- | ---: | ---: | ---: | --- |
| Python zipfile + hashlib + pathlib ????/????/???bytes?? | 1 | 0 | 0 | 15?????? ZIP ?????SHA-256????? |
| powershell -NoProfile -ExecutionPolicy Bypass -File agent/skills/zhihu/scripts/run.ps1 status | 1 | 0 | 0 | ??????? exit 0?installed=false / request_install_consent???? CLI ?? |
| CLI ??/??/??????? | 0 | 0 | 1 | ?????????????????? |
| ??????/support sync/@zhihu ?? | 0 | 0 | 1 | ????????12 Skill?????Skill?? |
| ? Skill ??????/???? | 0 | 0 | 1 | ??? |
| ?????? | 0 | 0 | 1 | REAL_HARDWARE_VALIDATION_PENDING |

Phase 1?2???0???4?????? vendor ??????????????????????????? TypeScript ???status ????????????
