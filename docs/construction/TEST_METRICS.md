...
## 2026-09-13 — Phase 18g 首启流程修订基线

| 检查 | 状态 |
| --- | --- |
| 人工点击模型页“配置 Key” | FAIL — IPC 返回“模型安全输入窗口未显示”，不能作为可用入口 |
| 首启预设遮罩 + 其他供应商分流 | NOT STARTED — 文档先行 |
| 双 Key 原生安全窗口 | NOT STARTED — 必测显示握手、DeepSeek 必填、Qwen 选填、取消、无 Secret 回显 |
| 预设/自定义首次成功后的重启 | NOT STARTED — 必须在重启后进入工作区选择 |
| 已配置冷启动 | NOT STARTED — 不得先闪模型页 |

本轮仍不使用真实付费 Key；原生窗口使用虚构测试输入，Secret 不写日志、截图或普通配置。
