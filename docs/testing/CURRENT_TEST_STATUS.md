...
对象：TASK-EXPLORE-006（Explore输出初始空态提取，2026-09-15）；本表明确区分本轮复测和未运行范围。
...
| Architecture / dev tooling | PASS | 四项AST、地图、20组工具测试；101个产品源文件 |
...
关键异常：首次专项因仓库根缺图片类型入口、工具测试因视觉变更未要求layout evidence而失败；显式复用Electron Vite声明并保守升级Explore styles/assets routing后33/33通过。隔离layout退出后Windows临时profile清理提示EPERM；Electron离线专项仍有os_crypt/GPU stderr但断言与退出码通过。历史Workbench/凭据真实流程仍待验。

本轮详细记录：[TASK-EXPLORE-006 evidence](evidence/TASK-EXPLORE-006.md)。此前构建记录：[治理 evidence](evidence/TASK-DEV-MAINT-001.md)（P3）。
