...
## 2026-09-13 — 学院呱呱知识库与教学演示升级

- [x] 重写当前软件使用知识源，模型配置提前并覆盖预设/其他供应商的完整用户操作与安全边界。
- [x] 教学演示升级至 v8/35 步，第一步先介绍整个 Catnip Forge 项目，再进入模型与各工作区教学。
- [x] 补全探索的知乎连接、找灵感、解问题、多会话历史、本地收藏、Context 选择与知识验证教学。
- [x] 补全线上 Skill Hub 的发现、下载、本地核对、同步与权限边界教学。
- [x] 静态、类型、构建、隔离教学 UI、真实 Electron 学院呱呱及模型中心验收通过。
- [x] 完整重建 Windows 解压包到 `electron/dist-package/Catnip Forge`，发布完整性、版本、官方知乎 Skill 与隔离首启门禁通过。
...
- [x] 针对 150% 缩放后的 `1280x720`、`1707x960`、`1707x1067` 有效视口适配顶栏三段、Agent 分栏、探索首页及两个探索流程。
- [x] `1280x720` 自动把 Agent 历史区收成 44px 图标轨道，对话输入框保持可见，探索阶段导航不会伸出标题框。
- [x] 多视口实测门禁通过：无页面/探索区横向越界，三个顶栏框、窗口控制、Agent 对话区及纵向滚动均可用。

## 2026-09-11 — v2.0.0 干净 Windows 包

- 从空 `electron/dist-package` 重建 `win-unpacked`，未复制工程 `.catnip`、收藏、探索/Agent 历史、日志、事件、录制、浏览器数据或真实 Key。
- 发布门禁和隔离 APPDATA 首启通过；首次使用状态、首启弹窗、Playwright 和品牌资源均已在真实成品进程验证。
- 成品共 41,927 个文件、4,502,224,578 字节；主程序 SHA-256 为 `F62CAD60A7E94BAD05D4D6CF209F912670C8A652A4DDB35EA5FF611E9A01E226`。
- 修正首启测试仍只查找旧聊天侧栏品牌图标的问题，使其适配默认进入探索后的顶部品牌，同时保留旧入口兼容断言。


## 2026-09-11 — 默认探索与任务工具主题收口

- 普通启动默认进入探索；保留 workbench smoke 的仓库测试入口。
- 删除与实时 EventBus 流重复的“完整日志”入口，保留任务记录行内“查看”用于单任务日志。
- 任务管理器按蓝紫参考重排 Build/Flash 色彩层级、操作标题与空状态；新增透明学院呱呱任务插画和三步引导。
- 监视器同步为深蓝紫输入区、配置区、状态与按钮主题。
- 类型检查、探索 UI 合约、Renderer 构建与真实桌面 CDP 交互验证通过；同尺寸视觉证据位于 `electron/.tmp/phase15-pass19-task-manager-comparison.png`。


## 2026-09-11 — 仓库页与工程目录同步

- 仓库页按深蓝 Skill Repository 参考重排，保留真实 Skill 数据并补齐筛选、搜索和排序交互。
- 硬件工程与参考代码改为始终展开，只列出真实一级工程目录；不再把目录中的源码递归铺平成列表。
- 两个资源区均增加“立即同步”和“打开目录”，分别复用工作台刷新与受控系统目录打开能力。
- 修复 1124px 视口下 Skills 行操作重叠及收起 Agent 后右侧仓库宽度被错误压缩的问题。

## 2026-09-11 — Explore 空草稿清理与记录重命名

- 返回探索首页时识别真正未操作的默认草稿并删除，避免“点开再返回”制造空历史；有输入、结果、对话、计划、交接或自定义名称的记录不会被误删。
- 最近探索记录增加行内重命名、保存和取消操作，名称持久化后重新打开仍保持。
- 会话自动保存改为使用独立标题状态；首次正式提交仍可从问题生成默认标题，但不会覆盖用户自定义名称。
...
- [x] 2026-08-07 用户截图复测揭示实际误启动旧 `dist-package-fixed`，其中仍为 14px/13px。现已删除旧并列包，packaged smoke 强制核对 `app.asar` 路径；专用光标端点 smoke 用用户 33 字符原句确认完整字体指标无差异、末端坐标差值 `0px`，并自动恢复原输入。
...
- [x] 对话输入区支持像微信桌面版一样拖动顶部横向分隔条调整高度，限制 64–320px、同步高亮层、支持键盘方向键微调并持久化本机高度
