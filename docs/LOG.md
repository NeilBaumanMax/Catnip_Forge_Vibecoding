...
## 2026-09-13 — 学院呱呱知识与教学演示补全

- 用户要求教学点击后的第一步先介绍整个项目，并把模型配置放到前面；本轮据此重排为项目总览、模型、Agent、仓库、探索、Skill Hub、硬件工具和助手的 35 步旅程。
- 软件指南已从旧启动/旧模型描述迁移到当前 safeStorage 与 Claude Code 供应商逻辑，并详细说明模型页每个关键字段、操作顺序和生效范围。
- 探索说明新增知乎连接生命周期、两个入口的用途差异、历史会话维护、收藏与 Context 的主动选择关系、知识验证记录；Skill Hub 明确“线上发现、本地管理”。
