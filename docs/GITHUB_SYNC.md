...
- GitHub SSH 访问已验证。
- 当前开发仓库为 `Catnip_Forge_Vibecoding`，主分支为 `main`，版本为 `1.0.0-7201`。`qwen_vision_attachments` 已于 2026-08-08 完成验收并合入主线。提交号和远端跟踪状态属于动态事实，必须以本地 `git log -1`、`git branch -vv` 和 `git remote -v` 为准，不再把历史 HEAD 写成当前值。
