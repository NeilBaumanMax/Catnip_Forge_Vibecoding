...
- 当前主线：`main`；`qwen_vision_attachments` 与历史施工分支 `electron_design` 均已合并。
- 产品：`Catnip Forge`；中文全称 `Catnip 硬件智能开发平台`；当前发布标签 `v1.5.0`（Build 7201）
...
- 仓库页以 Skills 为首要内容并固定排在工程资源之前；硬件工程和参考代码是默认折叠的双栏辅助组，共享展开状态，点击任意标题行都会让两栏同步开合并保持工具栏对齐；两栏各自保留项目数量和方向反馈，不显示 Agent 生成卡片，也不提供“导入文件夹”；Agent 工作区继续保留为编辑器受控根目录。
