...
- 仓库页以 Skills 为首要内容并固定排在工程资源之前；硬件工程和参考代码是默认折叠的辅助区，各自保留独立展开状态、项目数量和方向反馈，不显示 Agent 生成卡片，也不提供“导入文件夹”；Agent 工作区继续保留为编辑器受控根目录。
- 辅助资源折叠按钮使用完整标题行作为命中区域，提供 `aria-expanded` / `aria-controls`，展开和收起沿同一路径改变方向；`prefers-reduced-motion` 下禁用方向过渡。
...
界面验收至少确认：只有一个 Electron 主窗口；仓库页 Skills 位于首位，硬件工程与参考代码默认折叠且可独立展开，展开后文件条目仍可打开；打包版 Skills 路径包含 `resources/agent/skills` 且可由资源管理器打开，打开结果不挤压仓库标题；Build/Flash 刷新按钮和语义状态胶囊存在；清除前后的任务行数归零；监视器没有趋势图且左侧收发/右侧配置布局不变；应用内浅色/深色切换后控件可读且重载保持；外观按钮可拖动、重载保持位置、窗口缩放不越界，浮层在四个象限均不超出视口；外观按钮不遮挡编辑器字号工具条；COM 设备可枚举、打开后可关闭释放；编辑器右键菜单贴近指针；标签关闭按钮可见且有 hover/focus 反馈。
