...
8. 输入框使用与原生 textarea 同步的只读高亮层，将已识别的 `@skill-id` 呈现为圆角荧光笔色块；颜色由 Skill ID 稳定散列到调色板，同一 Skill 保持同色，不同 Skill 尽量区分。
...
`smoke:chat-ui` 在真实 Renderer 中两次打开完整 Skill 列表、分别插入不同的 `@skill-id`，确认每次选择后弹层关闭且焦点回到输入框；同时校验两个引用保留在正文原位、以不同的圆角荧光笔色块显示，并可用一次退格完整删除最后一个引用。
