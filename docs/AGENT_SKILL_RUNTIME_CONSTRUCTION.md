...
10. 输入区顶部提供整宽纵向拖拽热区，向上拖增高、向下拖减小，范围为 64–320px；外层编辑器是唯一高度真相，白框 textarea 与高亮层均使用绝对定位 `inset: 0` 完整填充，三者上下边界必须一致。分隔条支持键盘上下方向键每次调整 16px，并将最终高度保存在本机 `localStorage`。
